        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "recommend" in endpoint:
                    ...
                    if results:
                        print(
                            f"   🏆 Top result: {results[0].get('name')} (score: {results[0].get('score')})"
                        )
            else:
...
def main():
    ...
    service_url = run_command(
        "gcloud run services describe sun-chaser-staging --region=us-west1 --format='value(status.url)' 2>/dev/null"
    )
    ...
    print("\n📋 Manual steps:")
    print(
        "1. Visit GitHub Actions: https://github.com/Slybry2000/Sunchaserjuly27/actions"
    )
