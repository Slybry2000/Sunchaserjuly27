...
async def fake_weather(lat, lon):
    ...
    slots = [
        {"ts_local": f"2025-08-18T{str(i).zfill(2)}:00", "cloud_pct": 10, "temp_f": 65}
        for i in range(24)
    ]
...
def test_if_none_match_304():
    ...
    # inject dependency via FastAPI's dependency_overrides (consistent with other tests)
    ...
    resp2 = client.get(
        "/recommend?lat=47.6&lon=-122.3&radius=100", headers={"If-None-Match": etag}
    )
