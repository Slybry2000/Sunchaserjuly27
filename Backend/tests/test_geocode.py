...
async def test_geocode_success(httpx_mock):
    ...
    mock_response = {
        "features": [
            {
                "geometry": {
                    "coordinates": [-122.3328, 47.6061]  # [longitude, latitude] for Seattle
                }
            }
        ]
    }
    ...
    httpx_mock.add_response(
        url="https://api.mapbox.com/geocoding/v5/mapbox.places/Seattle, WA.json?access_token=test_token&limit=1",
        json=mock_response
    )
...
async def test_geocode_not_found(httpx_mock):
    ...
    httpx_mock.add_response(
        url="https://api.mapbox.com/geocoding/v5/mapbox.places/nonexistent place.json?access_token=test_token&limit=1",
        json=mock_response
    )
...
async def test_geocode_missing_token():
    ...
    with patch.dict('os.environ', {}, clear=True):
        with pytest.raises(ValueError, match="MAPBOX_TOKEN environment variable not set"):
