...
def test_dataset_size_and_schema():
    ...
    assert (
        len(locations) >= 100
    ), f"Dataset should have ≥100 locations, got {len(locations)}"
...
def test_category_diversity():
    ...
    assert (
        len(categories) >= 5
    ), f"Should have ≥5 categories, got {len(categories)}: {categories}"
    ...
    ), f"Should have common categories, found: {found_categories}"
...
def test_elevation_data():
    ...
    assert (
        max_elev - min_elev > 2000
    ), f"Should have elevation variety, range: {max_elev - min_elev}"
